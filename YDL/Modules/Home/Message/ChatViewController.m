...
//#import "ChatGroupDetailViewController.h"  // 群组
//#import "ChatroomDetailViewController.h"   // 聊天室
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSLog(@"%@ == == == %@", [EMClient sharedClient].currentUsername, self.conversation.conversationId);
    if (![[EMClient sharedClient].currentUsername isEqualToString:self.conversation.conversationId]) {
        
        // 删除好友
        EMError *error = [[EMClient sharedClient].contactManager deleteContact:self.conversation.conversationId isDeleteConversation:YES];
        if (!error) {
            NSLog(@"删除成功");
        }
        
        error = [[EMClient sharedClient].contactManager addContact:self.conversation.conversationId message:@"我想加您为好友"];
        if (!error) {
            NSLog(@"添加成功");
        }
    }
