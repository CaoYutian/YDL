...
+(void)createShowContent:(NSString *)content hiddenTime:(CGFloat)time
{
    ...
    weakSelf.bgView.frame = CGRectMake(0, 0, w+40, size.height+20);
    weakSelf.bgView.center = CGPointMake(weakSelf.center.x, weakSelf.center.y);
    weakSelf.contentlabel.frame = CGRectMake(5, 5, size.width, size.height);
    weakSelf.contentlabel.text = content;
    [weakSelf addHUDWithSelf];
    weakSelf.contentlabel.center = weakSelf.bgView.center;
    [UIView animateWithDuration:time animations:^{
        weakSelf.alpha = 0.0;
    } completion:^(BOOL finished) {
        if (finished) {
