...
#define LLNoDataSrcName(file) [UIImage imageNamed:file]
...
- (instancetype)initNoDataWithFrame:(CGRect)frame description:(NSString *)description canTouch:(BOOL)isCanTouch{
    ...
    if (self) {
        ...
        if (LLNoDataImageName.length>0) {
            ...
            iconImage = LLImageWithNamed(@"network_ signal");
        }
...
- (instancetype)initReloadBtnWithFrame:(CGRect)frame LLNoDataViewType:(LLNoDataViewType)type description:(NSString *)description reloadBtnTitle:(NSString *)title
{
    ...
    if (self) {
        ...
        if (type == LLNoInternet) {
            iconImage = LLImageWithNamed(LLNoInternetImageName.length>0?LLNoInternetImageName:@"network_ signal");
        }
        
        if (type == LLNoData) {
            iconImage = LLImageWithNamed(LLNoDataImageName.length>0?LLNoDataImageName:@"network_ signal");
        }
